...
try:
	print("## -- Get GUID for TP4SUT_RoomWall -----------------------------------------------")
	tpFmuPath = os.path.join(context,"RTT_TestProcedures","TP4SUT_RoomWall", "INTO-CPS-Demo.fmu")
	sutFmuPath = os.path.join(context,"RTT_TestProcedures","SUT_FCU_Ctrl", "INTO-CPS-Demo_sut.fmu")
	#roomFmuPath = os.path.join(context, "..", "..", "FMUs", "RoomHeating.fmu")
	roomFmuPath = os.path.join(context,"RTT_TestProcedures","RoomWall", "RoomHeating.fmu")
	tpGuid = get_guid (tpFmuPath) 
	sutGuid = get_guid (sutFmuPath) 
	roomGuid = get_guid (roomFmuPath) 

	jsonTemplate = os.path.join(context,"sut","SUT_roomwall_template.json")
	jsonOut = os.path.join(context,"sut","SUT_roomwall.json")

	with open(jsonTemplate, "rt") as fin:
		with open(jsonOut, "wt") as fout:
			for line in fin:
				#print "#-0 {0}".format(line)
				l = line.replace('{1841d970-559b-11e7-aa29-080027489296}', tpGuid)
				#print "#-1 {0}".format(l)
				l = l.replace('{dd9122b1-55d9-11e7-8f55-080027489296}', sutGuid)
				#print "#-2 {0}".format(l)
				l = l.replace('{c2d275e0-33c1-48ef-88e0-66c4daa921d0}', roomGuid)
				#print "#-3 {0}".format(l)
				fout.write(l)
